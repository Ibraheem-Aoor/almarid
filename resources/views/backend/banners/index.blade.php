...
<script>
	$(document).ready(function() {
		...
		TABLE.DataTable({
			...
			@if(isset($data['lang']) && !empty($data['lang']) && in_array($data['lang'],Config::get('app.locales')))
			ajax: '{!! url(config('app.prefix','admin').'/'.$data['route'].'/data?lang='.$data['lang']) !!}',
			@else
			...
			columns: [
				...
				{className: 'text-center', data: 'type', name: 'type', searchable: true,orderable: true},
...
//                $('.textarea').wysihtml5();
	...
	function postData(form){
		...
		$.ajax({
			...
			error:function(data) {
				console.log(data);
				showAlertMessage('alert-danger', data.responseJSON.message);
				// showAlertMessage('alert-danger', 'الرجاء ارفاق ملف بصيغة مدعومة - صورة أو فيديو فقط');
			} ,
			statusCode: {
				500: function(data) {
					console.log(data);
			...
			<div class="row">
				<div class="col-xs-12">
													...
													<div class="col-md-12">
														<div class="form-body">
															<div class="row">
																...
																<div class="col-md-12">
																	<div class="form-group form-md-line-input has-success">
																		<label class="col-md-2 control-label" for="to_date">القسم</label>
																		<div class="col-md-10">
																			<select name="type" class="form-control">
																				<option value="ACCESSORY">الاكسسوارات</option>
																		...
																		<div class="col-md-10">
																			<select name="lang" class="form-control">
																				@foreach(Config::get('app.locales') as $row)
																					<option  value="{{ $row }}">{{ trans('locales.'.$row) }}</option>
																				@endforeach
																			</select>
																			<div class="form-control-focus">
																			</div>
					...
					<form action="" method="get" class="pull-left" style="margin-right: 30px;">
						<select name="lang" class="pull-left form-control" style="width: 120px;padding: 0;margin-left: 5px;">
							@foreach(Config::get('app.locales') as $row)
								<option @if(isset($data['lang']) && !empty($data['lang']) && in_array($data['lang'],Config::get('app.locales')) && $data['lang'] == $row) selected @endif value="{{ $row }}">{{ trans('locales.'.$row) }}</option>
							@endforeach
						</select>
					...
					<div class="box">
						<div class="box-body">
							<table class="table table-striped table-bordered table-hover" id="objects_table">
								<thead>
								<tr>
									<th class="text-center" width="30%">العنوان</th>
									<th class="text-center" width="20%">القسم</th>
