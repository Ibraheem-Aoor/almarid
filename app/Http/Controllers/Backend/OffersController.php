...
class OffersController extends BackendController {
    ...
    var $title = 'العروض والخصومات';
    var $route = 'offers';
    var $view_folder = 'offers';
    var $upload_folder = 'offers';
    private $model = null;
    ...
    public function data(Request $request){
        $objects = $this->model->with('product')->select('*');


        return \Datatables::of($objects)
            ->addColumn('name', function ($object) {
                if(isset($object->product) && !empty($object->product) && isset($object->product->name)){
                    return $object->product->name;
                }
                return '<span style="color: #f00;">السيارة اما محذوفة او معطلة</span>';
            })
...
//            ->make(true);
    ...
    public function add(Request $request){
        ...
        $this->model->where('product_id',$product_id)->update(['status'=>0]);
    ...
    public function change_status(Request $request){
        ...
        if($object->status == 1){
            $status = 'تفعيل؟';
            $this->model->where('product_id',$object->product_id)->update(['status'=>0]);
            $object->update(['status'=>0]);
            Product::where('id',$object->product_id)->update([
                'is_offer'=>0,
                'offer_id'=>0,
                'offer_price'=> 0,
            ]);
