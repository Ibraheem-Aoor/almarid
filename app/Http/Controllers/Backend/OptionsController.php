...
class OptionsController extends BackendController {
    ...
    var $title = 'خيارات المنتجات';
    var $route = 'options';
    var $view_folder = 'options';
    var $upload_folder = 'options';
    private $model = null;
    ...
    public function index(Request $request){
        ...
        $this->data['categories'] = Category::where('type','OPTIONS')->where('status',1)->get();
    ...
    public function data(Request $request){
        $objects = $this->model;
        if($request->has('type')){
            $objects = $objects->where('category_id',$request->get('type'));
        }
