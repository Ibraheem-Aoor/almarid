...
use App\Models\ExportProductColorImage;
use Illuminate\Http\Request;
use App\Models\ExportProduct;
use App\Models\WebExportProductColor;


class ExportProductsImagesController extends BackendController {

    var $title = 'ألوان وصور سيارات التصدير';
    var $route = 'webexportcars';
    var $view_folder = 'webexportcars';
    var $upload_folder = 'products';
    private $model = null;

    public function __construct(){
        parent::__construct();
        $this->model = new WebExportProductColor();
    }
    ...
    public function index(){
       ...
       $this->data['products'] = ExportProduct::get();
        return view('backend/'.$this->view_folder.'/index')->with('data',$this->data);
    }

    public function data(){
        $objects =  $this->model->select('*')->with('color');
        return \Datatables::of($objects)
            ...
            ->addColumn('product', function ($object) {return ExportProduct::find($object->export_product_id)->name;})
    ...
    public function add(Request $request){
        ...
        $object_product = ExportProduct::find($product_id);
        $update_product = [
            'is_web'=>1,
        ];
        ...
        if($request->hasfile('images'))
        {
            foreach($request->file('images') as $image)
            {
                ...
                ExportProductColorImage::create([
                    'web_export_product_color_id' => $object->id,
                    'image' => $image_path
                ]);
            }
        }
        ...
        if($object){
            ...
            $object_row.='<td class="text-center">'.ExportProduct::find($object->export_product_id)->name.'</td>';
    ...
    public function show_edit_form(Request $request){
        if($request->has('id')){
            ...
            $this->data['product_images'] = ExportProductColorImage::where('web_export_product_color_id',$request->input('id'))->get();
            $this->data['route'] = $this->route;
    ...
    public function delete_image(Request $request,$id){
        $this->data = $request->all();
        if(intval($id) > 0){
            $deletedRestaurant = ExportProductColorImage::find($id);
            if($deletedRestaurant){
